...
class SingleLayerNet:
    ...
    def train(self, inputs, outputs):
        ...
        while nr_iterations > 0:
            for (x, t) in t_set:
                ...
                perceptrons_w += np.array([ou * x * learning_rate for ou in np.array(t_m - a_m)])
                perceptrons_b += (t_m - a_m) * learning_rate
            nr_iterations -= 1
        self.perceptrons_w = perceptrons_w
        self.perceptrons_b = perceptrons_b
        pickle.dump(zip(perceptrons_w, perceptrons_b), open('pick_percep.pickle', 'wb'))
    ...
    def test_one(self, input_data):
        ...
        arr = self.activation_arr(z_m)
        return self.out_classes[np.argmax(arr)]
    ...
    def show(self):
        for i, w in enumerate(self.perceptrons_w):
            ...
            plt.show()
            # i mg.save(str(title) + ".png")
        # for w in self.perceptrons_w:
        #     side = int(sqrt(num_of_edges))
        #     arr = [w[i*side:(i+1)*side] for i in range(side)]
        #     print(arr)
        #     img = Image.fromarray(np.array(arr), 'RGB')
        #     img.show()
        #     plt.imshow()
